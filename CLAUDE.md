...
- 訪問診療を行う事業所数: 地方厚生局「届出受理医療機関名簿」のうち在宅療養支援診療所・病院の「機能強化型」（類型1・2、緊急往診や在宅看取りの実績基準あり）のみを対象。類型3（基本型）は実績基準がなく実際に訪問診療を行っているとは限らないため除外。緯度経度が無いため住所から市区町村を抽出しNominatimでジオコーディング。全国8地方厚生局すべてのデータに対応、`home-visit-clinic-data.js`に格納
